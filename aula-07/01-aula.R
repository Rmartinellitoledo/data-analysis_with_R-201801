...
#' Nesta simulação, identifique quantas sequências de 6 falhas ocorreram. 
#' Use como _seed_ os últimos 5 dígitos da sua matrícula. 
#' Veja no exemplo anterior o uso da função `rle`.
set.seed(30267)
grafico_ligacoes <- data_frame(x = 0:20, y=pgeom(0:20, prob = 0.1) * 100)

ggplot(grafico_ligacoes, aes(x=x, y=y)) +
  geom_col() +
  scale_x_continuous(name = "Tentativas até ser atendido", breaks=0:20) +
  scale_y_continuous(name = "Prob (%)") +
  theme_light()

rbernoulli(500, p = 0.1) -> Randomicos_Seed
seq_Randomicos_Seed <- rle(Randomicos_Seed)
data.frame(logico = seq_Randomicos_Seed$values,
           Qtd = seq_Randomicos_Seed$lengths) %>%
  filter(logico == FALSE) %>%
  filter(Qtd == 6) %>%
  count(logico)
...
#'     + Qual a probabilidade de um usuário acertar o CAPTCHA após 5 tentativas fracassadas? 
# 70%, pois os fracassos não interferem no resultado posterior, segundo as informações do enunciado.

#'     Qual o mínimo de tentativas para que a probabilidade seja maior que 50%?
# Uma, pois o usuário já parte de uma probabilidade de 70%.

#'     + Você observou que, das últimas 500 _tentativas_ de publicação de reclamações, 340 acertaram a validação de CAPTCHA. 
#'     Qual a probabilidade de uma quantidade entre 320 e 350 tentativas passarem pela validação de CAPTCHA a cada 500 tentativas? 
pbinom(320:350, size = 500, prob = 0.70, lower.tail = FALSE)
#'     Dada a probabilidade de 70% de sucesso, qual o número esperado de publicações a cada 500 CAPTCHAS? DICA: ESTAMOS TRATANDO DA DISTRIBUIÇÃO BINOMIAL.
sum(rbernoulli(500, p = 0.7))
